...
if (isset($update['callback_query'])) {
    ...
    if (preg_match('/^c:(\d+):(none|\d+)$/', $data, $m) && $chatId) {
        ...
        $stmt = $db->prepare('UPDATE confirmations SET user_id=:uid, username=:un, confirmed_option=:opt, expires_on=:iso, confirmed_at=:ts WHERE id=:id');
        $stmt->bindValue(':uid', $uid, SQLITE3_INTEGER);
        $stmt->bindValue(':un', $uname, SQLITE3_TEXT);
        $stmt->bindValue(':opt', $pickedLabel, SQLITE3_TEXT);
        $stmt->bindValue(':iso', $iso, SQLITE3_TEXT);
        $stmt->bindValue(':ts',  $nowIso, SQLITE3_TEXT);
        $stmt->bindValue(':id',  $cid, SQLITE3_INTEGER);
...
if (isset($update['message']['photo'])) {
    ...
    $stmt = $db->prepare('INSERT INTO confirmations (chat_id, tg_file_id, gcs_uri, ocr_text, options_json) VALUES (:chat, :tg, :gcs, :ocr, :opts)');
    if (!$stmt) {
        $x = "Erro: " . $db->lastErrorMsg();
        tgApi('sendMessage', ['chat_id'=>$chatId, 'text'=>$x]);
        exit;
    }
    $stmt->bindValue(':chat', (string)$chatId, SQLITE3_TEXT);
    $stmt->bindValue(':tg',   $fileId,        SQLITE3_TEXT);
    $stmt->bindValue(':gcs',  $gcsUri,        SQLITE3_TEXT);
    $stmt->bindValue(':ocr',  $full,          SQLITE3_TEXT);
    $stmt->bindValue(':opts', json_encode($options, JSON_UNESCAPED_UNICODE), SQLITE3_TEXT);
    $stmt->execute();
